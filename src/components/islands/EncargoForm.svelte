...
<script lang="ts">
	...
	type CustomItem = { productId: string; productName: string; quantity: number };

		// Start with two rows: first preselected, second empty
		let fields: CustomItem[] = [
			{ productId: products[0]?.id || '', productName: products[0]?.name || '', quantity: 1 },
...
<form on:submit|preventDefault={handleSubmit} class="encargo-form">
	...
	<style>
		.encargo-form { display: grid; gap: 0.75rem; }
		.encargo-field { display:flex; gap:0.5rem; }
		select { flex: 1; padding:0.5rem; border-radius:0.5rem; border:1px solid #ddd }
		input[type="number"] { width:100px; padding:0.4rem; border-radius:0.4rem; border:1px solid #ddd }
